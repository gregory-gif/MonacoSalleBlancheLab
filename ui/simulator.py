...
class SimulationWorker:
    ...
    def run_session(current_ga: float, overrides: StrategyOverrides, tier_map: dict, use_ratchet: bool = False, penalty_mode: bool = False, active_level: int = 1, mode: str = 'Standard'):
        ...
        if is_active_penalty:
            # ADAPTATION: If min bet is €100 (Monaco), we stick to that if possible
        ...
        for m in range(total_months):
            ...
            if can_play:
                ...
                for _ in range(sessions_this_month):
                    ...
                    # CAPTURE DATA FOR YEAR 1 (First 12 months)
...
def show_simulator():
    ...
    def save_current_strategy():
        ...
        config = {
            ...
            'tac_mode': select_engine_mode.value, 
    ...
    def update_ladder_preview():
        try:
            ...
            for level, t in t_map.items():
                ...
                rows.append({
                    'tier': f"Tier {level}",
                    'bet': f"€{t.base_unit} (Press +{t.press_unit})", 
                    'start': start_str,
                    'risk': f"{risk_pct:.1f}%"
                })
            ...
            for i in range(0, config['num_sims'], batch_size):
                ...
                def run_batch_careers():
                    ...
                    for k in range(count):
                        should_track = (i == 0 and k == 0) # Track only first sim
    ...
    def render_analysis(results, config, start_ga, overrides):
        ...
        # --- FLIGHT RECORDER CONTAINER (DEDICATED) ---
                ...
                if y1_log:
                    ...
                    for entry in y1_log:
                        ...
                        table_rows.append({
                            ...
                            'Hands': f"{entry.get('hands', 0)}"
                        })
                    
                    ui.aggrid({
                        'columnDefs': [
                            {'headerName': 'Mo', 'field': 'Month', 'width': 60},
                            {'headerName': 'PnL', 'field': 'Result', 'width': 90, 'cellClass': "font-bold"},
                            {'headerName': 'Tot. Bal', 'field': 'Balance', 'width': 100},
                            {'headerName': 'Game Bal', 'field': 'Game Bal', 'width': 100, 'cellClass': "text-yellow-400"},
                            {'headerName': 'Hands', 'field': 'Hands', 'width': 80},
                        ],
                        'rowData': table_rows,
                        'domLayout': 'autoHeight',
                    }).classes('w-full theme-balham-dark')
                else:
                    ui.label("⚠️ NO DATA: PLEASE RESTART PYTHON SCRIPT").classes('text-red-400 font-bold p-4')
    ...
    with ui.column().classes('w-full max-w-4xl mx-auto gap-6 p-4'):
        # TITLE CHANGE: v2.3 to confirm Monaco update
        ui.label('RESEARCH LAB v2.3 (MONACO FINAL)').classes('text-2xl font-light text-blue-300')

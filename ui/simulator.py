...
def show_simulator():
    ...
    def render_analysis(stats, config, start_ga, overrides, all_results):
        ...
        with report_container:
            ...
            # Progression mapping
            press_map = {0: 'Flat', 1: 'Press 1-Win', 2: 'Press 2-Wins', 3: 'Progression 100-150-250', 4: "Capped D'Alembert (Strategist)", 5: "La Caroline (1-1-2-3-4)", 6: "Negative Caroline (1-1-2-3-4)", 7: "Negatif 1-2-4-7 Snap-Back", 8: "The Gentle Surgeon (1-2-4)", 9: "Winner's Guard (1-1-2-4)", 10: "Negatif Profit Guard (1-2-4)"}
        ...
        with ui.card().classes('w-full bg-slate-900 p-6 gap-4'):
            ...
            with ui.grid(columns=2).classes('w-full gap-8'):
                with ui.column():
                    ...
                    slider_base_bet = ui.slider(min=100, max=500, step=100, value=100, on_change=update_ladder_preview).props('color=purple'); lbl_base.bind_text_from(slider_base_bet, 'value', lambda v: f'€{v}')
                ...
                with ui.column():
                    ...
                    ui.separator().classes('bg-slate-700 my-2')
                    ui.label('🎯 FIBONACCI HUNTER [1-1-2-3-5-8]').classes('text-xs text-amber-400 font-bold')
                    switch_fibonacci = ui.switch('Enable Fibonacci Hunter').props('color=amber')
                    switch_fibonacci.value = False
                    with ui.row().classes('w-full items-center gap-2'):
                        ui.label('Base Unit').classes('text-xs text-slate-400')
                        slider_fib_base = ui.slider(min=50, max=500, value=100, step=50).props('color=amber')
                        ui.label().bind_text_from(slider_fib_base, 'value', lambda v: f'€{int(v)}')
                    select_fib_mode = ui.select({'STOP_SESSION': 'Sniper (Exit on Win)', 'RESET_AND_CONTINUE': 'Marathon (Continue)'}, value='STOP_SESSION', label='Mode').classes('w-full')
                    ui.label('Win sequence = +20× base unit profit').classes('text-xs text-amber-300 italic')

...
def show_simulator():
    ...
    def update_ladder_preview():
        ...
        for level, t in t_map.items():
            risk_pct = (t.base_unit / t.min_ga) * 100 if t.min_ga > 0 else 100
            rows.append({
                'tier': f"Tier {level}",
                'bet': f"€{t.base_unit}",
                'start': f"€{t.min_ga:,.0f}",
    ...
    def render_analysis(results, config, start_ga, overrides):
        ...
        # Scoring Logic
        # We penalize cost, but if the goal is Gold, high cost might be necessary.
        # Adjusted scoring to respect Gold Probability more.
        if real_monthly_cost <= 0: score_cost = 100
        else: score_cost = max(0, 100 - (real_monthly_cost / 5)) 
            
        score_time = active_pct
        score_gold = gold_prob
        
        total_score = (score_gold * 0.40) + (score_survival * 0.30) + (score_cost * 0.10) + (score_time * 0.20)
        ...
        with report_container:
            ...
            try:
                ...
                # REPORTING FIX: Correctly describe Cap Logic
                if config['cap_tier2']:
                    lines.append(f"Betting Cap: PROTECTED BERSERKER (Max €100)")
                    lines.append(f"   -> Cruise: €100 when > €2000")
                    lines.append(f"   -> Defense: €50 when < €2000")
                else:
                    lines.append(f"Betting Cap: OFF (Standard Exponential)")
                ...
                # REPORTING FIX: Correctly describe Ratchet Logic
                if overrides.ratchet_enabled:
                    ...
                    elif overrides.ratchet_mode == "Gold Grinder":
                        lines.append(f"   Ladders: +8>3, +12>5 (Reset on Drop)")
                        lines.append(f"   Note: Ignores stop loss on profit drop (Shoes 1-2) to max volume.")
                else:
